...
async(req:Request, res:Response, next:NextFunction)=>{
    try{
        ...
        if(!foundUser.length) next( new BadRequestError('Invalid Email/Password Combination'));
        const user = foundUser[0];
        const { id, email, firstName, lastName, role } = user;
        let isMatch = await user.comparePassword(req.body.password);
        if(!isMatch) return next( new BadRequestError('Invalid Email/Password Combination'));
        let token = jwt.sign(
            {
                exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 30, //30days
                id,
